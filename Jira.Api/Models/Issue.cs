...
public class Issue : IRemoteIssueFieldProvider
{
	...
	private void Initialize(RemoteIssue remoteIssue)
	{
		...
		DueDate = remoteIssue.duedate;
		Updated = remoteIssue.updated;
		ResolutionDate = remoteIssue.resolutionDateReadOnly;
		SecurityLevel = remoteIssue.securityLevelReadOnly;
		...
		var affectsVersions = OriginalRemoteIssue.affectsVersions ?? Enumerable.Empty<RemoteVersion>();
		AffectsVersions = new ProjectVersionCollection("versions", Jira, Project, [.. affectsVersions.Select(v =>
		{
			v.ProjectKey = OriginalRemoteIssue.project;
			return new ProjectVersion(Jira, v);
		})]);

		var fixVersions = OriginalRemoteIssue.fixVersions ?? Enumerable.Empty<RemoteVersion>();
		FixVersions = new ProjectVersionCollection("fixVersions", Jira, Project, [.. fixVersions.Select(v =>
		{
			v.ProjectKey = OriginalRemoteIssue.project;
			return new ProjectVersion(Jira, v);
		})]);

		var labels = OriginalRemoteIssue.labels ?? [];
		Labels = new IssueLabelCollection([.. labels]);

		var components = OriginalRemoteIssue.components ?? Enumerable.Empty<RemoteComponent>();
		Components = new ProjectComponentCollection("components", Jira, Project, [.. components.Select(c =>
		{
	...
	/// </summary>
	public JiraClient Jira { get; }

	/// <summary>
	/// Gets the security level set on the issue.
	/// </summary>
	public IssueSecurityLevel SecurityLevel { get; private set; }
	...
	/// </summary>
	public ComparableString Key { get; private set; }
	...
	public ComparableString? this[string customFieldName]
	{
		get
		{
			var customField = CustomFields[customFieldName];
		...
		set
		{
			...
			{
				CustomFields.Add(customFieldName, customFieldValue, null);
			}
		}
	}
	...
	public async Task<Issue> SaveChangesAsync(CancellationToken cancellationToken)
	{
		...
		if (string.IsNullOrEmpty(OriginalRemoteIssue.key))
		{
			var newKey = await Jira.Issues.CreateIssueAsync(this, cancellationToken).ConfigureAwait(false);
			serverIssue = await Jira.Issues.GetIssueAsync(newKey, cancellationToken).ConfigureAwait(false);
		}
		else
		{
			await Jira.Issues.UpdateIssueAsync(this, cancellationToken).ConfigureAwait(false);
			serverIssue = await Jira.Issues.GetIssueAsync(OriginalRemoteIssue.key, cancellationToken).ConfigureAwait(false);
		}
	...
	public Task<IEnumerable<IssueRemoteLink>> GetRemoteLinksAsync(CancellationToken cancellationToken)
	{
		...
		return Jira.RemoteLinks.GetRemoteLinksForIssueAsync(OriginalRemoteIssue.key, cancellationToken);
	}
	...
	public async Task WorkflowTransitionAsync(string actionNameOrId, WorkflowTransitionUpdates? additionalUpdates, CancellationToken cancellationToken)
	{
		...
		await Jira.Issues.ExecuteWorkflowActionAsync(this, actionNameOrId, additionalUpdates, cancellationToken).ConfigureAwait(false);
		var issue = await Jira.Issues.GetIssueAsync(OriginalRemoteIssue.key, cancellationToken).ConfigureAwait(false);
	...
	public Task<IPagedQueryResult<Issue>> GetSubTasksAsync(int skip, int? take, CancellationToken cancellationToken)
	{
		...
		return Jira.Issues.GetSubTasksAsync(OriginalRemoteIssue.key, skip, take, cancellationToken);
	}
	...
	{
		var attachments = fileInfos.Select(f => new UploadAttachmentInfo(f.Name, Jira.FileSystem.FileReadAllBytes(f.FullName))).ToArray();
	...
	/// <param name="data">The byte array containing the attachment data.</param>
	/// <param name="cancellationToken">Cancellation token for this operation.</param>
	...
	public Task AddAttachmentAsync(UploadAttachmentInfo[] attachments, CancellationToken cancellationToken)
	{
		...
		return Jira.Issues.AddAttachmentsAsync(OriginalRemoteIssue.key, attachments, cancellationToken);
	}
	...
	public Task DeleteAttachmentAsync(Attachment attachment, CancellationToken cancellationToken)
	{
		...
		return Jira.Issues.DeleteAttachmentAsync(OriginalRemoteIssue.key, attachment.Id, cancellationToken);
	}
	...
	public Task<IDictionary<string, IssueFieldEditMetadata>> GetIssueFieldsEditMetadataAsync(CancellationToken cancellationToken)
	{
		...
		return Jira.Issues.GetFieldsEditMetadataAsync(OriginalRemoteIssue.key, cancellationToken);
	}
	...
	public Task<IEnumerable<IssueChangeLog>> GetChangeLogsAsync(CancellationToken cancellationToken)
	{
		...
		return Jira.Issues.GetChangeLogsAsync(OriginalRemoteIssue.key, cancellationToken);
	}
	...
	public Task<IEnumerable<Comment>> GetCommentsAsync(CancellationToken cancellationToken)
	{
		...
		return Jira.Issues.GetCommentsAsync(OriginalRemoteIssue.key, cancellationToken);
	}
	...
	public Task<IEnumerable<Comment>> GetCommentsAsync(CommentQueryOptions options, CancellationToken cancellationToken)
	{
		...
		return Jira.Issues.GetCommentsAsync(OriginalRemoteIssue.key, options, cancellationToken);
	}
	...
	public Task DeleteCommentAsync(Comment comment, CancellationToken cancellationToken)
	{
		...
		return Jira.Issues.DeleteCommentAsync(OriginalRemoteIssue.key, comment.Id, cancellationToken);
	}
	...
	public Task<string[]> GetLabelsAsync(CancellationToken cancellationToken)
	{
		...
		return Jira.Issues.GetLabelsAsync(OriginalRemoteIssue.key, cancellationToken);
	}
		...
		CancellationToken cancellationToken)
	{
		...
		return Jira.Issues.AddWorklogAsync(OriginalRemoteIssue.key, worklog, worklogStrategy, newEstimate, cancellationToken);
	}
		...
		CancellationToken cancellationToken)
	{
		...
		return Jira.Issues.DeleteWorklogAsync(OriginalRemoteIssue.key, worklog.Id, worklogStrategy, newEstimate, cancellationToken);
	}
	...
	public Task<IEnumerable<Worklog>> GetWorklogsAsync(CancellationToken cancellationToken)
	{
		...
		return Jira.Issues.GetWorklogsAsync(OriginalRemoteIssue.key, cancellationToken);
	}
	...
	public async Task RefreshAsync(CancellationToken cancellationToken)
	{
		...
		var serverIssue = await Jira.Issues.GetIssueAsync(OriginalRemoteIssue.key, cancellationToken).ConfigureAwait(false);
		Initialize(serverIssue.OriginalRemoteIssue);
	}
	...
	public Task<IEnumerable<IssueTransition>> GetAvailableActionsAsync(CancellationToken cancellationToken)
	{
		...
		return Jira.Issues.GetActionsAsync(OriginalRemoteIssue.key, cancellationToken);
	}

	/// <summary>
	/// Gets the workflow actions that the issue can be transitioned to including the fields that are required per action.
	/// </summary>
	/// <param name="expandTransitionFields">Whether to expand the transition fields in the response.</param>
	/// <param name="cancellationToken">Cancellation token for this operation.</param>
	public Task<IEnumerable<IssueTransition>> GetAvailableActionsAsync(bool expandTransitionFields, CancellationToken cancellationToken)
	{
		...
		return Jira.Issues.GetActionsAsync(OriginalRemoteIssue.key, expandTransitionFields, cancellationToken);
	}
	...
	public Task<IssueTimeTrackingData> GetTimeTrackingDataAsync(CancellationToken cancellationToken)
	{
		...
		return Jira.Issues.GetTimeTrackingDataAsync(OriginalRemoteIssue.key, cancellationToken);
	}
	...
	public Task AddWatcherAsync(string usernameOrAccountId, CancellationToken cancellationToken)
	{
		...
		return Jira.Issues.AddWatcherAsync(OriginalRemoteIssue.key, usernameOrAccountId, cancellationToken);
	}
	...
	public Task<IEnumerable<JiraUser>> GetWatchersAsync(CancellationToken cancellationToken)
	{
		...
		return Jira.Issues.GetWatchersAsync(OriginalRemoteIssue.key, cancellationToken);
	}
	...
	public Task DeleteWatcherAsync(string usernameOrAccountId, CancellationToken cancellationToken)
	{
		...
		return Jira.Issues.DeleteWatcherAsync(OriginalRemoteIssue.key, usernameOrAccountId, cancellationToken);
	}
	...
	public async Task AssignAsync(string assignee, CancellationToken cancellationToken)
	{
		...
		await Jira.Issues.AssignIssueAsync(Key.Value, assignee, cancellationToken);
		var issue = await Jira.Issues.GetIssueAsync(OriginalRemoteIssue.key, cancellationToken).ConfigureAwait(false);
		Initialize(issue.OriginalRemoteIssue);
	}
	...
	public Task<ReadOnlyDictionary<string, JToken>> GetPropertiesAsync(IEnumerable<string> propertyKeys, CancellationToken cancellationToken)
	{
		...
		return Jira.Issues.GetPropertiesAsync(OriginalRemoteIssue.key, propertyKeys, cancellationToken);
	}
	...
	public Task SetPropertyAsync(string propertyKey, JToken obj, CancellationToken cancellationToken)
	{
		...
		return Jira.Issues.SetPropertyAsync(OriginalRemoteIssue.key, propertyKey, obj, cancellationToken);
	}
	...
	public Task DeletePropertyAsync(string propertyKey, CancellationToken cancellationToken)
	{
		...
		return Jira.Issues.DeletePropertyAsync(OriginalRemoteIssue.key, propertyKey, cancellationToken);
	}
	...
	async Task<RemoteFieldValue[]> IRemoteIssueFieldProvider.GetRemoteFieldValuesAsync(CancellationToken cancellationToken)
	{
		...
		foreach (var localProperty in typeof(Issue).GetProperties())
		{
			...
			else
			{
				...
				var remoteStringValue = await GetStringValueForPropertyAsync(OriginalRemoteIssue, remoteProperty, cancellationToken).ConfigureAwait(false);
	...
	internal async Task<RemoteIssue> ToRemoteAsync(CancellationToken cancellationToken)
	{
		...
		if (Resolution != null)
		{
			await Resolution.LoadIdAndNameAsync(Jira, cancellationToken).ConfigureAwait(false);
			remote.resolution = new RemoteResolution { id = Resolution.Id, name = Resolution.Name };
		}

		if (Priority != null)
		{
			await Priority.LoadIdAndNameAsync(Jira, cancellationToken).ConfigureAwait(false);
